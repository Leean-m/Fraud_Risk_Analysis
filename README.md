...
- Fraud represents a relatively small share of total transactions, but remains a persistent risk within the system.  
- Fraud is not evenly distributed across categories, with certain transaction types showing significantly higher fraud rates.  
- Transaction size influences fraud vulnerability, suggesting that some payment values are more exposed to fraudulent activity.  
...
- Demonstrating how SQL can be used to uncover actionable fraud insights  
...
This project demonstrates how SQL can be used to analyse credit card transaction data and uncover meaningful fraud risk patterns. The findings show that fraud is concentrated in specific areas rather than evenly distributed, supporting the need for more targeted and data-driven fraud prevention strategies.
